...
Dans la [partie précédente](./dependency_Inversion_1) nous avons défini le notion d'inversion de dépendances. Dans cette seconde partie, nous l'améliorons.
...
C'est pour cette dernière raison principalement que nous _placerons toujours l'interface dans le module le plus stable_. A noter que le module le plus stable n'est pas forcément le module de plus haut niveau (voir architecture hexagonale).
