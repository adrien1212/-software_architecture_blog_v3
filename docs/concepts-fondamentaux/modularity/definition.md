...
:::definition
 Modularity is the property of a system to which degree it is composed of modules. Modularity is recursive - Modules may be further decomposed into submodules. A module is a cohesive, loosely coupled, encapsulated and composable piece of software that does one thing.[^1]
...
## Caractéristiques
Un module est donc un concept qui abstrait notre système et qui répond aux caractéristiques suivantes 
...
- **Encapsulation de l’Implémentation**
  Un module conserve sa logique interne privée. Les autres parties du système ne voient que ce que le module choisit d’exposer. Cette encapsulation permet de maintenir un **faible couplage** entre les modules.
...
Dans la section [Caractéristiques d'un module](./characteristics) nous verrons comment implémenter ces caractéristiques dans nos solutions
...
La modularité est ainsi au cœur de la conception des architectures. Lorsqu'on souhaite réaliser un logiciel avec une architecture Microservices ou Modular Monolithique la première chose à faire est de définir des [Bounded Context](../bounded_context) afin d'avoir des services/modules faiblement couplés.
