...
Nous avons déjà évoqué la notion de Service au sens SOA/Microservice. Mais aucune de ces notions ne correspond au *Domain Service*.
...
Si l’on déplace toute la logique métier dans le Domain Service, celui-ci devient une simple façade procédurale dépourvue de véritable encapsulation d’état. On obtient alors un Anemic Domain Model — un anti-pattern où les entités du domaine se réduisent à de simples structures de données sans comportement, contredisant les principes du Domain-Driven Design.
